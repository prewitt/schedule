...
class FileBrowserSite(object):
    ...
    def filebrowser_view(self, view):
        #return staff_member_required(never_cache(view)) #pangwei
    ...
    def get_urls(self):
        ...
        urlpatterns = patterns('',
            ...
            url(r'^download/$', file_exists(self, path_exists(self, self.filebrowser_view(self.download))), name="fb_download"),
    ...
    def get_task_path(self,request,path):
        ...
        for item in pathList:
            if item !='':
                strP+=item+'/'
                site.createTaskDir(strP)
                site.task_name=item
            i+=1
        return HttpResponseRedirect("/filebrowser/browse/")

    def browse_file(self, request):
        """
        Browse Files/Directories.
        """
        filter_re = []
        ...
        filelisting = FileListing(path,
            ...
            sorting_order=query.get('ot', DEFAULT_SORTING_ORDER),
            site=self)
        # print filelisting._fileobjects_total
        files = []
        if SEARCH_TRAVERSE and query.get("q"):
            listing = filelisting.files_walk_filtered()
        else:
        ...
        return render_to_response('filebrowser/index.html', {
            ...
            'settings_var': get_settings_var(directory=self.directory),
            'breadcrumbs': get_breadcrumbs(query, query.get('dir','')),
            'breadcrumbs_firstTask':self.task_name,
            'breadcrumbs_title': '',
            'filebrowser_site': self
        }, context_instance=Context(request, current_app=self.name))
    ...
    def createTaskDir(self, server_path):
        """
    ...
    def download(self,request,buf_size=262144):
        """
        Download existing File/Directory.
        """
        query = request.GET
        path = os.path.join(self.downLoadPath, self.directory,query.get('dir', ''))
        ...
        def readFile(fn, buf_size=262144):
            ...
            while True:
                ...
                if c:
                    ...
                    break
            f.close()
        response = HttpResponse(readFile(filepath))
